...
class HomeScreenResponse {
  ...
  factory HomeScreenResponse.fromJson(Map<String, dynamic> json) => HomeScreenResponse(
    status: json["status"],
    featuredPresenters: List<Presenter>.from(json["featured_presenters"].map((x) => Presenter.fromJson(x))),
    topCategories: List<Category>.from(json["top_categories"].map((x) => Category.fromJson(x))),
    dealsOfTheDay: List<Product>.from(json["deals_of_the_day"].map((x) => Product.fromJson(x))),
    recommendedProducts: List<Product>.from(json["recommended_products"].map((x) => Product.fromJson(x))),
    newArrivals: List<Product>.from(json["new_arrivals"].map((x) => Product.fromJson(x))),
    featuredBrands: List<FeaturedBrand>.from(json["featured_brands"].map((x) => FeaturedBrand.fromJson(x))),
    liveStreams: List<LiveStream>.from(json["live_streams"].map((x) => LiveStream.fromJson(x))),
  );

  Map<String, dynamic> toJson() => {
    "status": status,
    "featured_presenters": List<dynamic>.from(featuredPresenters.map((x) => x.toJson())),
    "top_categories": List<dynamic>.from(topCategories.map((x) => x.toJson())),
    "deals_of_the_day": List<dynamic>.from(dealsOfTheDay.map((x) => x.toJson())),
    "recommended_products": List<dynamic>.from(recommendedProducts.map((x) => x.toJson())),
    "new_arrivals": List<dynamic>.from(newArrivals.map((x) => x.toJson())),
    "featured_brands": List<dynamic>.from(featuredBrands.map((x) => x.toJson())),
